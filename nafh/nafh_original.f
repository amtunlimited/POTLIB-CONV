      subroutine apot(r,e,De,nt,nsurf)

c this subroutine returns adiabatic energies and adiabatic
c derivatives

      implicit none

      integer nt,nsurf,i,j
      double precision De(3,nt), r(nt,3), e(nt),rn(3),De1(3),De2(3),
     &    De3(3),diff,ddiff,e1,e2,e3

      do i=1,nt
        rn(1)=r(i,1)
        rn(2)=r(i,2)
        rn(3)=r(i,3)
        call pot(rn,e1,De1,nt,1)
        call pot(rn,e2,De2,nt,2)
        call pot(rn,e3,De3,nt,3)

        diff = dsqrt((e1-e3)**2+4.0d0*e2**2)
        if (nsurf .eq. 1) then
          e(i) = 0.5d0*(e1+e3-diff)
        elseif (nsurf .eq. 2) then
          e(i) = 0.5d0*(e1+e3+diff)
        else
          write(*,*)'nsurf ill defined in apot'
          stop
        endif
        do j= 1,3
          ddiff = ((e1-e3)*(De1(j)-De3(j))+4.0d0*e2*De2(j))*(1/diff)
C                 (e1*De1(j) + e3*De3(j) -e1*De3(j) - e3*De1(j) +
C     &         4.0d0*e2*De2(j))/diff
          if (nsurf .eq. 1) then
            De(j,i) = 0.5d0*(De1(j)+De3(j)-ddiff)
          elseif (nsurf .eq. 2) then
            De(j,i) = 0.5d0*(De1(j)+De3(j)+ddiff)
          endif
        enddo
...
C  This is version 1.0 of the potential surfaces for the two lowest electronic
C  states of NaFH
c
c  The potential is called by first issuing a call to PREPOT which initializes
c  all surfaces, then by calling POT 
c
      subroutine prepot
      ...
      if( g_myrank .eq. 0 )then
         write(6,100)
         write(6,*)
     &    'PREPOT called for NaFH version 1.0 potential energy surface.'
         write(6,*)'Last modified April 16, 1997.'
      endif
...
C     NaF coupling:
         write(6,101)(ccNF(i),i=1,4)
         write(6,102)(ccNHa(i),i=1,4)
         write(6,103)(ccNHi(i),i=1,4)
         write(6,104)(ccNHi1(i),i=1,4)
         write(6,105)xpNH,xpNH1
         write(6,106)(ahf(i),i=1,3)
         write(6,107)(anaf(i),i=1,3)
         write(6,108)(anah(i),i=1,3)
         write(6,109)(r21(i),i=1,3)
         write(6,110)(r23(i),i=1,3)
         write(6,111)(r13(i),i=1,3)
      endif
...
C     Ground NaF surface
         write(6,101)DeNF,reNF,(cNF(i),i=1,6)
C----------------------------------
C     Ground HF suface:
         write(6,103)DeHF,reHF,(cHF(i),i=1,6)
C----------------------------------
         ...
         write(6,119)(c2b(j),j=1,4)
         write(6,120)(c2c(j),j=1,4)
         write(6,121)(r20i(j),j=1,4)
         write(6,122)r20a
         write(6,123)(al20i(j),j=1,4)
         write(6,124)(al20i1(j),j=1,4)
         write(6,125)al20a
         write(6,126)(al2a(j),j=1,4)
         write(6,127)(rr0a(j),j=1,4)
         write(6,128)psi
         write(6,129)rsi
         write(6,130)(dip2(j),j=1,4)
         write(6,131)(adip2(j),j=1,4)
         write(6,132)(rdip2(j),j=1,4)
         write(6,133)(acut2(j),j=1,4)
         write(6,134)(rcut2(j),j=1,4)
         write(6,135)(ry(j),j=1,4)
         write(6,136)(aly(j),j=1,4)
         write(6,137)(dy(j),j=1,4)
         write(6,138)(dip2n(j),j=1,4)
         write(6,139)(adip2n(j),j=1,4)
         write(6,140)(rdip2n(j),j=1,4)
         write(6,141)(acut2n(j),j=1,4)
         write(6,142)(rcut2n(j),j=1,4)
      endif
C
      return
c
