...
c     Initializations and onetime calculations
  ...
  100 format(/71('*')/)
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      CHARACTER*75 REF(5)
      double precision De(3,1), e(1),De1(3),De2(3),
     &    De3(3),diff,ddiff,e1(1),e2(1),e3(1)
      PARAMETER (NATOM=25,N3ATOM=3*NATOM)
      PARAMETER (ISURF=5,JSURF=ISURF*(ISURF+1)/2)
      COMMON /USRICM/ CART(NATOM,3),ANUZERO,NULBL(NATOM),     
     X                NFLAG(20),NASURF(ISURF+1,ISURF+1),NDER  
...
c     Start of original code
      ...
      call pot2(r,e1,De1,1,1)
      call pot2(r,e2,De2,1,2)
      call pot2(r,e3,De3,1,3)

      diff = dsqrt((e1(1)-e3(1))**2+4.0d0*e2(1)**2)
      
      ENGYGS = 0.5d0*(e1(1)+e3(1)-diff)
      do j= 1,3
        ddiff = ((e1(1)-e3(1))*(De1(j)-De3(j))
     &          +4.0d0*e2(1)*De2(j))*(1/diff)
...
c     NOTE: original code had many data blocks already, so in the intrest of
c           and reduced possibility of error, it was decided to simply copy
c           instead of tryig consolidate all of the data blocks into this one
         
      END
c     This space is for other subroutines from the original code

      subroutine pot2(r,e,De,nt,nsurf)
C
