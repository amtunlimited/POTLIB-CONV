...
C Sys.:     NH3
c name:     nh3
C           Ref.: B. Maessen, P. Bopp, D. R. McLaughlin, & M. Wolfsberg
C                 Z. Naturforsch. Teil A 39, 1005-1006 (1984).
c form:     ?
c coor:     Cartesian in original input
c features: ?
c****************************************************         
      SUBROUTINE PREPOT
...
c****************************************************         
     ...
     *                  C4A
      ...
      REF(1)= 'NH3 B. Maessen,P. Bopp,D. R. McLaughlin,& M. Wolfsberg'
      REF(2)= '    Z. Naturforsch. Teil A 39, 1005-1006 (1984)'
      REF(3)= ' '
      REF(4)= ' '
      REF(5)= ' '
      IRCTNT = 4
...
c****************************************************         
      ...
      do i = 1,4
         ...
         enddo
      enddo
      call surf(engygs,xb,degsdr,12)
c      call surf(engygs,xb,dxb,12)
c      write(6,*) 'POT derivatives in au'
c      do i = 1,12
c         degsdr(i) = dxb(i)
c      enddo
c      do i = 1,10,3
...
c****************************************************         
      ...
      data re/1.024D0/
      data a,b,c/0.53190D-11,1.1406D-11,0.6140D0/
C QUADRATIC TERMS: C2
      data C2A, C2B, C2C, C2D, C2E, C2F/3.52595D0, 0.22163D0,-0.22163D0,
     *                                  0.01480D0,-0.11620D0, 0.05810D0/
C CUBIC TERMS: C3
      data C3A, C3B, C3C, C3D, C3E, C3F, C3G, C3H/
     *     -0.55000D0,-0.54000D0,-7.01000D0,-0.03778D0,0.05667D0,
     *     -0.22667D0,-0.07000D0, 0.07000D0/
C QUARTIC TERM: C4A
      data C4A/11.07417D0/
      END
