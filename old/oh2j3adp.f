...
C
      ...
      COMMON /PT34CM/ IPRT
      COMMON /PT35CM/ EASYAB, EASYBC, EASYAC
      COMMON /J3BDCM/ D(3), BETA, REQ(3), A, GAMP(3), RBB
C
...
C
   ...
   50 CONTINUE
      ...
      RBB = RBB /BOHR
C
C   Initialize the variables for the energy in the three asymptotic valleys
C
      EASYAB = D(1)
      EASYBC = D(2)
      EASYAC = D(3)
...
C       THE COLLINEAR GEOMETRY AND CALL POT2D
C
         RACTMP = R(3)
         R(3) = R(2) + R(1)
         CALL POT2D
...
C
     ...
     1 0.50D+2,0.60D+2,0.70D+2,0.80D+2,0.90D+2/
      DATA DSP/0.10947D+3,0.107466D+3,0.104362D+3,0.98983D+2,
     1 0.96437D+2,0.96731D+2,0.100108D+3,0.103761D+3,
     2 0.105184D+3,0.105982D+3,0.10656D+3/
      DATA BSP/1.9444D+0,1.9076D+0,1.7729D+0,1.5517D+0,1.4393D+0,
     1 1.4263D+0,1.6255D+0,1.8765D+0,2.0749D+0,2.2302D+0,2.2957D+0/
...
C
       DIF = R(2) - REHH
       IF (DIF*BHH .LT. -70.0D0) WRITE (6,60)R(1),R(2),R(3),DIF
 60   FORMAT(2X,' R(1), R(2), R(3), DIF',4F12.4)
      EXP1 = EXP(-BHH*DIF)
      TERM = 1.D+0 - EXP1
      ENERGY = DHH*TERM**2
      IF (NDER .EQ. 1) THEN
...
C
      DIF = R(1) - REOH
      IF (DIF*BOH.LT.-70.0D+0) WRITE(6,60)R(1),R(2),R(3),DIF
      EXP2 = EXP(-BOH*DIF)
      TERM = 1.D+0 - EXP2
...
C
      ...
      END IF
C
C     Three atom dissociation limit
C
