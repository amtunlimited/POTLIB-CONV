...
C   Common name:     LSTH
C   Reference:       B. Liu, J. Chem. Phys., Vol. 58, 1925(1973);
C                    B. Liu and P. Siegbahn, D. G. Truhlar and C. J. Horowitz,
C                    J. Chem. Phys., Vol. 68, 2457(1978); 
C                    D. G. Truhlar and C. J. Horowitz, 
C                    J. Chem. Phys. 71, 1514(E) (1979)
C
C   PREOT must be called once before any calls to POT.
...
C                  NDER                1
C                  IPRT                6
C   LIU-SIEGBAHN-TRUHLAR-HOROWITZ POTENTIAL FOR H3, INCLUDING ANALYTIC
C    DERIVATIVES.
C   THIS IS TRUE LSTH WITH H2 SPLINE FIT CURVES.
C    ENERGY: 1 HARTREE = 27.21161 EV = 2625.500 KJ/MOL.
C    LENGTH: 1 BOHR = 0.5291771E-10 M.
C   NOTE- THIS IS NOT AN OPTIMIZED CODE FROM THE POINT OF VIEW OF
C         COMPACTNESS OF THE CODING.
C   FOR MORE EFFICIENT CALCULATION OF THE ENERGY, REMOVE THE DERIVATIVES.
C
...
C
    ...
    5 CALL SPL1D2(87,RKW,EKW,WKW,1,R,S1)
      ...
      IF(IC.LT.30) GO TO 7
      WRITE(IPRT,6000)
6000  FORMAT(/,2X,T5,'Error: In PREPOT the minimum of VH2 ',
     *               'could not be found',/)
      STOP 'PREPOT 1'
    7 R = R - S1(2)/S1(3)
      GO TO 5
...
C WB DERIVATIVES
C
   30 WB1P=(RSV(1)/RSV(3)-1.D0)/RSV(2)-1.D0/RSV(3)-(COS2+COS3)/RSV(1)
...
1     IF (R(1).GT.10.D0) CALL VBIGR(R(1),S1)
      IF (R(1).GT.10.D0) GO TO 2
      CALL SPL1D2(87,RKW,EKW,WKW,1,R(1),S1)
    2 IF (R(2).GT.10.D0) CALL VBIGR(R(2),S2)
      IF (R(2).GT.10.D0) GO TO 3
      CALL SPL1D2(87,RKW,EKW,WKW,1,R(2),S2)
    3 IF (R(3).GT.10.D0) CALL VBIGR(R(3),S3)
      IF (R(3).GT.10.D0) RETURN
