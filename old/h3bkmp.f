...
C
      ...
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
      COMMON /PT31CM/ R(3), ENERGY, DEDR(3)
      COMMON /PT32CM/ NSURF, NDER, IDBUG, NFLAG(20)
      COMMON /PT34CM/ IPRT
      COMMON /PT35CM/ EASYAB, EASYBC, EASYAC
      DIMENSION RVAL(3)
      RVAL(1) = R(1)
...
C
     ...
     .           +  dCal(3) + dCas(3) + dCbnda(3) + dCbndb(3)
      ...
      if(ipr.gt.0) then
          WRITE(IPRT,*)
          WRITE(IPRT,*) 'h3tot enter --------------------------------'
          WRITE(IPRT,7000) r,icompc,vtot,
     .             vlon,vas,cal,cas,vbnda,vbndb,cbnda,cbndb
          if(ipr.gt.1)then
             WRITE(IPRT,7100) dVtot,dVlon,dVas,dVbnda,dVbndb,
     .                           dCal,dCas,dCbnda,dCbndb
          end if
          WRITE(IPRT,7400) (vb(1,i),i=1,5),(vb(2,i),i=1,5)
          WRITE(IPRT,7500) (cb(1,i),i=1,9),(cb(2,i),i=1,9)
          WRITE(IPRT,7999) Vbnda,Vbndb,Cbnda,Cbndb,  
     .                  dVbnda,dvbndb,dCbnda,dCbndb
          WRITE(IPRT,*) 'exiting subr.h3tot'
          WRITE(IPRT,*) 'h3tot exit ---------------------------------'
      end if
      return
...
C    
      ...
      if(ipr.gt.0) WRITE(IPRT,7100) e3
...
C        call triplet(r(i),etrip,2)
         ...
         if(ipr.gt.0) WRITE(IPRT,7400) i,esing,etrip
         e3(i) = etrip(1)
         de3(i) = etrip(2)
         q(i)  = half*(E1(i) + E3(i))  
         j(i)  = half*(E1(i) - E3(i)) 
...
702   CONTINUE
     ...
     .        + abs( j(3)-j(1) )**2 
      ...
      Vlon   = sumQ - rootJt   
      if(ipr.gt.0) then
         WRITE(IPRT,7410) sumq,sumj
         WRITE(IPRT,7420) vlon,rootJt
      end if
C
...
C
     ...
     .         - 0.25d0*(two*j(3)-j(1)-j(2))*(dE1(3)-de3(3))/rootJt
      if(ipr.gt.0) then
         WRITE(IPRT,7000) r,e1,e3,vlon
         WRITE(IPRT,7100) q,j
         WRITE(IPRT,7200) dVlon
      end if
...
C
     ...
     .             -aa6*aa7*A2*exp6/R
      if(ipr.gt.0) WRITE(IPRT,7100) Vas,dS,dVas
...
C
      ...
      if(ipr.gt.0) WRITE(IPRT,7100) A,dA
...
C
     ...
     .           + A*(u9/Pr2 + u10/Pr + u11 + u12*Pr + u13*Pr2)
      ...
      if(ipr.gt.0) then
            ...
            WRITE(IPRT,*) 'dT(i) = ',dT
            WRITE(IPRT,*) ' id = ',id
            WRITE(IPRT,7400) series,term1,eterm,Cas
            WRITE(IPRT,*) ' Cas = ',Cas
      end if
C
...
705      CONTINUE
         if(ipr.gt.0) then
C           WRITE(IPRT,7500) dPr,dt,dterm1,determ,dseries,dCas
            WRITE(IPRT,7500) dPr,dt,dterm1,determ,dseris,dCas
            WRITE(IPRT,*) ' dCas = ',dCas
         end if
      end if
      return
...
706   CONTINUE
      ...
      if(ipr.gt.0)then
         WRITE(IPRT,*)
         WRITE(IPRT,7100) Cal
         WRITE(IPRT,7000) G,dG,sumv,dCal
         WRITE(IPRT,*) 'exiting subr.csym'
      end if
...
cd     if(ipr.gt.1)then
cd         WRITE(IPRT,*)
cd         WRITE(IPRT,*) ' from subr.vbcb -----------------------------'
cd         WRITE(IPRT,6000) rpass
cd         WRITE(IPRT,6010) b1a,b1b
cd         WRITE(IPRT,6020) db1a,db1b
cd         if(ipr.gt.2)then
cd            WRITE(IPRT,7000) dc1dr1,dc1dr2,dc1dr3,
cd    .                     dc2dr1,dc2dr2,dc2dr3,
cd    .                     dc3dr1,dc3dr2,dc3dr3
cd            WRITE(IPRT,7010) t1,t2,t3,c1,c2,c3,d1,d2,d3
cd         end if
cd     end if
C
...
C
cd     if(ipr.gt.1)then
cd        WRITE(IPRT,7650) B2,B3
cd        WRITE(IPRT,7700) db2,db3
cd        WRITE(IPRT,*)
cd        WRITE(IPRT,*) 'Vbend values :'
cd        k=1
cd        WRITE(IPRT,7150) k,dvb1,dvb2,dvb3,dvb4,dvb5,dVbnda
cd        WRITE(IPRT,7155) (vb(1,i),i=1,5)
cd        k=2
cd        WRITE(IPRT,7150) k,dvb1,dvb2,dvb3,dvb4,dvb5,dVbndb
cd        WRITE(IPRT,7155) (vb(2,i),i=1,5)
cd        WRITE(IPRT,7100) Vbnda,Vbndb
cd        WRITE(IPRT,*)
cd        WRITE(IPRT,*) 'Cbend values:'
cd        WRITE(IPRT,*) 'c81,c81,c82=',c81,c81,c82
cd        WRITE(IPRT,*) 'c91,c83,c84=',c91,c83,c84
cd        WRITE(IPRT,*) 'd81,d81,d82=',d81,d81,d82
cd        WRITE(IPRT,*) 'd91,d83,d84=',d91,d83,d84
cd        k=1              
cd           WRITE(IPRT,2020) k,(cb(1,j),j=1,9)
cd           WRITE(IPRT,2033) sumT,Cbnds(1)
cd        k=2              
cd           WRITE(IPRT,2020) k,(cb(k,j),j=1,9)
cd           WRITE(IPRT,2033) sumT,Cbnds(2)
cd        k=1
cd        WRITE(IPRT,7152) k,dcb1,dcb2,dcb3,dcb4,dcb5,dcb6,
cd    .               dcb7,dcb8,dcb9,(dCbnds(1,i),i=1,3)
cd        k=2
cd        WRITE(IPRT,7152) k,dcb1,dcb2,dcb3,dcb4,dcb5,dcb6,
cd    .                 dcb7,dcb8,dcb9,(dCbnds(k,i),i=1,3)
cd        WRITE(IPRT,7200) dT,dCbnda,dCbndb
cd        WRITE(IPRT,*) 'VbCb  exit -------------------------'
cd        WRITE(IPRT,*)
cd     end if
      return
